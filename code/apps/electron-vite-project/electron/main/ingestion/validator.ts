 ...
 * Produces ValidatedCapsule on success — the ONLY module that can construct this type.
 *
 * Fail-closed: ANY structural violation → rejection.
 *
 * SHALL NOT: check handshake state, resolve policy, enforce sharing mode,
 *            verify sender domain, execute capsules, or persist state.
 */
...
const VALID_EXTERNAL_PROCESSING: ReadonlySet<string> = new Set([
  'none', 'local_only',
])

const VALID_CLOUD_PAYLOAD_MODES: ReadonlySet<string> = new Set([
  'none', 'snippet', 'full',
])
...
function measureJsonDepth(value: unknown, currentDepth: number = 0): number {
  ...
  for (const key of Object.keys(value as Record<string, unknown>)) {
    max = Math.max(max, measureJsonDepth((value as any)[key], currentDepth + 1))
    if (max > INGESTION_CONSTANTS.MAX_JSON_DEPTH) return max
  }
  return max
}
...
function countFields(value: unknown, limit: number, count: { n: number }): boolean {
  ...
  for (const key of Object.keys(value as Record<string, unknown>)) {
    ...
    if (!countFields((value as any)[key], limit, count)) return false
  }
  return true
}

/**
 * Create a safe copy of a payload object with poisoned keys removed.
 * Prevents prototype pollution from __proto__, constructor, prototype keys.
 */
...
/**
 * Public API — the ONLY function that returns ValidatedCapsule.
 */
...
function runValidation(candidate: CandidateCapsuleEnvelope): ValidationResult {
  // Step 1: Check ingestion error flag
  ...
  // Prototype pollution check — reject if any poisoned key is an OWN property
  if (
  ...
  if ('external_processing' in obj && obj.external_processing !== undefined) {
    ...
    if (!VALID_EXTERNAL_PROCESSING.has(obj.external_processing) && !obj.external_processing.match(/^[a-z0-9_-]+$/i)) {
      return fail('INVALID_ENUM_VALUE', `Invalid external_processing: ${obj.external_processing}`)
    }
  }
  ...
  // Step 8: Cryptographic field presence (NOT verification — handshake does that)
  ...
  if ('prev_hash' in obj && obj.prev_hash !== undefined && typeof obj.prev_hash === 'string') {
    ...
    if (obj.prev_hash.length !== 64) {
      return fail('HASH_BINDING_MISMATCH', `prev_hash wrong length: expected 64, got ${obj.prev_hash.length}`)
    }
  }

  // Step 10: Payload size
  ...
  // All checks passed — sanitize and construct ValidatedCapsule
...
/**
 * Private factory — NOT exported. Only called from within this module.
 */
