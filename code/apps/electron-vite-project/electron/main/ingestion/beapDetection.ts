 ...
 *
 * If input looks like BEAP but JSON parsing fails → malformed (not silent discard).
 */

import type { RawInput, BeapDetectionResult, DetectionMethod } from './types'
...
function checkJsonStructure(input: RawInput): BeapDetectionResult | null {
  ...
  try {
    ...
    // Not valid JSON — not BEAP by structure
  }
...
function tryParseBeapJson(
  ...
  try {
    ...
    return { detected: false, malformed: true, detection_error: err?.message ?? 'JSON parse failed' }
  }
}
