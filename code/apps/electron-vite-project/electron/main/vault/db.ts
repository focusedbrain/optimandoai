...
export function listVaultsForAccount(session: SessionUserInfo | null): ListVaultsAccountResult {
  ...
  console.log(
    ...
    JSON.stringify({
      ...
      visibleVaultIds: vaults.map((v) => v.id),
      hiddenForeignVaults: hiddenForeignCount,
      legacyUnclaimedVaults: legacyUnclaimed.length,
      legacyUnclaimedVaultIds: legacyUnclaimed.map((v) => v.id),
    }),
  )
...
/**
 * Stamp account ownership on a legacy vault (meta JSON + vaults.json registry).
 * Caller must verify passphrase unwraps the vault DEK before calling.
 */
export function writeVaultOwnerClaim(vaultId: string, owner: VaultOwnerRecord): void {
  const metaPath = getVaultMetaPath(vaultId)
  if (!existsSync(metaPath)) {
    const e = new Error(`Vault metadata file missing for vault "${vaultId}"`)
    ;(e as { code?: string }).code = 'ERR_VAULT_CLAIM_WRITE_FAILED'
    throw e
  }
  ...
  try {
    ...
    const e = new Error(`Cannot read vault metadata for "${vaultId}": ${msg}`)
    ;(e as { code?: string }).code = 'ERR_VAULT_CLAIM_WRITE_FAILED'
    throw e
  }

  meta.owner_wrdesk_user_id = owner.owner_wrdesk_user_id
  meta.owner_sub = owner.owner_sub
  meta.owner_iss = owner.owner_iss
  meta.owner_email = owner.owner_email
  if (owner.owner_email_verified !== undefined) {
    meta.owner_email_verified = owner.owner_email_verified
  }
  meta.owner_claimed_at = owner.owner_claimed_at
  meta.vault_schema_version = owner.vault_schema_version

  try {
    writeFileSync(metaPath, JSON.stringify(meta, null, 2), 'utf-8')
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err)
    const e = new Error(`Failed to write vault metadata for "${vaultId}": ${msg}`)
    ;(e as { code?: string }).code = 'ERR_VAULT_CLAIM_WRITE_FAILED'
    throw e
  }

  updateVaultRegistryOwner(vaultId, owner)
  console.log(
    '[VAULT_CLAIM]',
    JSON.stringify({
      vaultId,
      owner_wrdesk_user_id: owner.owner_wrdesk_user_id.slice(0, 64),
      owner_claimed_at: owner.owner_claimed_at,
    }),
  )
}

/** Merge owner fields onto an existing vaults.json row (or append if missing). */
export function updateVaultRegistryOwner(vaultId: string, owner: VaultOwnerRecord): void {
  const registryPath = getVaultRegistryPath()
  ...
  if (existsSync(registryPath)) {
    try {
      ...
      console.warn('[VAULT DB] Failed to read registry for owner update, creating new one')
    }
  }

  const row = registry.vaults.find((v) => v.id === vaultId)
  if (row) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err)
    const e = new Error(`Failed to update vault registry for "${vaultId}": ${msg}`)
    ;(e as { code?: string }).code = 'ERR_VAULT_CLAIM_WRITE_FAILED'
    throw e
  }
}
