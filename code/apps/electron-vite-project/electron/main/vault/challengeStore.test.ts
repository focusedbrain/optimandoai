...
// ---------------------------------------------------------------------------
// Integration-style: simulate the passkey replay scenario
// ---------------------------------------------------------------------------
describe('Passkey replay scenario', () => {
  it('same challenge cannot be used for two unlock-complete calls', () => {
    const store = new ChallengeStore()

    // Server issues challenge in beginPasskeyUnlock
    const challenge = store.issue('challenge-from-begin')

    // First completePasskeyUnlock — valid
    expect(store.consume(challenge)).toBe(true)

    // Attacker replays the same payload — rejected
    expect(store.consume(challenge)).toBe(false)
  })

  it('same challenge cannot be used for two enroll-complete calls', () => {
    const store = new ChallengeStore()

    const challenge = store.issue('enroll-challenge')

    // First completePasskeyEnroll — valid
    expect(store.consume(challenge)).toBe(true)
  ...
  it('challenge from one flow cannot be used in another', () => {
    const store = new ChallengeStore()

    store.issue('enroll-challenge')
    store.issue('unlock-challenge')

    // Use enroll challenge
    expect(store.consume('enroll-challenge')).toBe(true)

    // Try to reuse enroll challenge for unlock — rejected
    expect(store.consume('enroll-challenge')).toBe(false)

    // unlock challenge still works
    expect(store.consume('unlock-challenge')).toBe(true)
  })
})
