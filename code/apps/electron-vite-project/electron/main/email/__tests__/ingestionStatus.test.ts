 ...
 * combinations of:
 *   • resolveIngestionOwnership() output (mocked)
...
vi.mock('../ingestionOwnership', () => ({
  resolveIngestionOwnership: () => mockResolveIngestionOwnership(),
}))
...
describe('resolveIngestionStatus — IngestionStatusCode mapping', () => {
  ...
  it('OK_SINGLE_MACHINE — host owns, polling normally', () => {
    ...
    const res = resolveIngestionStatus(['acc1'])
  ...
  it('OK_SINGLE_MACHINE — no accounts provided, still resolves correctly', () => {
    mockResolveIngestionOwnership.mockReturnValue(singleMachineOwnership())
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map())

    const res = resolveIngestionStatus([])
    expect(res.code).toBe('OK_SINGLE_MACHINE')
  ...
  it('PAUSED_HOST_DELEGATED — host correctly not polling, sandbox not yet confirmed', () => {
  ...
  it('PAUSED_HOST_DELEGATED — sandbox owns, consent present, but no poll has run yet', () => {
  ...
  it('ACTION_NEEDED_READ_CONSENT — sandbox owns, read consent missing for one account', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    ...
    const res = resolveIngestionStatus(['acc1', 'acc2'])
  ...
  it('ACTION_NEEDED_READ_CONSENT — maps to sandboxIngestion held_read_consent_missing', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
  ...
  it('OK_SANDBOX_FETCHING — sandbox owns, last poll ok, no held messages', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
  ...
  it('OK_SANDBOX_FETCHING — sandbox poll ok with zero fetched (inbox empty, still ok)', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true)
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map([
  ...
  it('PAUSED_SANDBOX_UNREACHABLE — sandbox owns, last poll held_fetch_failed', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true) // consent present
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map([
  ...
  it('PAUSED_SANDBOX_UNREACHABLE — held_no_custody_key maps to unreachable (not consent)', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true) // consent present
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map([
  ...
  it('DEGRADED_HELD_MESSAGES — fetching ok but individual messages held in depackage', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true)
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map([
  ...
  it('DEGRADED_HELD_MESSAGES — ok on acc1 but acc2 also ok; held on acc2 triggers DEGRADED', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true)
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map([
  ...
  it('INV-5 — result never contains error message text, only error count', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true)
  ...
  it('result always includes resolvedAt timestamp', () => {
    mockResolveIngestionOwnership.mockReturnValue(singleMachineOwnership())
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map())
  ...
  it('accounts array mirrors the accountIds input order', () => {
    mockResolveIngestionOwnership.mockReturnValue(sandboxOwnsOnSandbox())
    mockHasRoleScopedTokens.mockReturnValue(true)
    mockGetLastSandboxPollOutcomes.mockReturnValue(new Map([
      ['acc1', pollOutcome('ok')],
    ...
    const res = resolveIngestionStatus(['acc3', 'acc1', 'acc2'])
