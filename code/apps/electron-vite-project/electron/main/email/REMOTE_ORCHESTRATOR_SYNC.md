...
On **`registerInboxHandlers`**, **`setSimpleOrchestratorRemoteDrainPrimary(true)`** is set. Then **`scheduleOrchestratorRemoteDrain`** is a **no-op** and **`drainOrchestratorRemoteQueueBounded`** returns immediately (no legacy `setImmediate` chain, no inline bounded batches). A **10s `setInterval`** in `ipc.ts` **ticks** often but **starts a batch only when `simpleDrainNextBatchAllowedAt`** has passed (after-batch pause is **provider-specific**: ~**5s** for Microsoft 365 / Gmail, ~**30s** when the batch includes IMAP work). Each batch loads up to **45** candidates, orders **non-IMAP first**, then takes at most **20** rows with at most **10** **IMAP** rows. **Per-IMAP-account LIST ping** defers that account’s rows for the tick if ping fails. **`forceReconnect`** only when the cached IMAP session is **not** `isConnected()` (not every batch). **Inter-row delay** ~**200ms** (API) vs ~**1s** (IMAP); **transient** backoff ~**3s** vs ~**60s** (IMAP). Two consecutive IMAP apply failures trigger a **5-minute** cool-down gate. Then **`applyOrchestratorRemoteOperation`**, SQLite updates, **`inbox:drainProgress`** (**`batchMoved` / `batchSkipped` / `batchErrors` / `batchImapDeferred`**), **`inbox:simpleDrainRow`**. Permanent / max-attempts → **`failed`**. **`inbox:debugTestMoveOne`** still uses **`processOrchestratorRemoteQueueBatch`**.
...
- **Pre-move idempotent skip (TEMPORARY off):** `applyOrchestratorRemoteOperation` used to short-circuit with `{ ok: true, skipped: true }` when **`imapVerifyMessageInMailbox`** saw the RFC `Message-ID` in the destination — false positives on some hosts left INBOX unchanged. **Pre-move** check is commented out until confirmed; **post-move** verification after `MOVE` remains.
...
- **Verify remote (UI):** 🔧 Debug → **Verify remote** → `inbox:verifyImapRemoteFolders` — LIST + STATUS counts + lifecycle exact-match snapshot (read-only, no CREATE). IPC **`Promise.race`** **15s** timeout so a dead session cannot hang the panel; activity log shows **`Verify Remote: FAILED — …`**.
...
**`inbox:fullRemoteSyncAllAccounts`** (UI **☁ Sync Remote**): **first** **`clearAllPullActiveLocks`** (stale **`syncPullLock`** entries from a crashed Pull would otherwise defer every queue row for that account); (1) **`markOrphanPendingQueueRowsAsFailed`** for `pending`/`processing` rows whose **`account_id`** is not a connected gateway account (or all such rows when no accounts); (2) **`enqueueUnmirroredClassifiedLifecycleMessages`** — classified **`inbox_messages`** with no **`completed`/`pending`/`processing`** queue row; (3) **`enqueueFullRemoteSync`** per connected account; (4) **`scheduleOrchestratorRemoteDrain`** only — **does not** await **`drainOrchestratorRemoteQueueBounded`** (no 150-batch / 28s cap on IPC). **`syncPullLock`** also **expires** locks held longer than **5 minutes**. Background drain + **15s watchdog** (`ensureOrchestratorRemoteDrainWatchdog`, registered with the **same `getDb` as inbox handlers**) resets **`processing`** rows stuck **>2 minutes** back to **`pending`**, calls **`forceDrainRestart`** + **`scheduleOrchestratorRemoteDrain`** whenever **`pending > 0`**. Each drain batch emits **`inbox:drainProgress`** to the renderer (debug activity log). On inbox handler registration, a **startup** pass runs **`enqueueUnmirroredClassifiedLifecycleMessages`** once if anything was missing.
