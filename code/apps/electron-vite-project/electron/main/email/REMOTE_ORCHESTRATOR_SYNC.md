...
| Pending review | `pending_review` | Add user label `Pending Review`, remove `INBOX` + `Pending Delete` label | **Move** → folder named `Pending Review` (default): find under Inbox (paginated children), else root `mailFolders?$filter=displayName`, else **POST /me/mailFolders** (top-level), else Inbox child create. **Never DELETE.** | `MOVE` → `Pending Review` (default mailbox; locate source via `imap_remote_mailbox` + RFC Message-ID search) |
| Pending delete | `pending_delete` | Add user label `Pending Delete`, remove `INBOX` + `Pending Review` label | Same as pending review for folder **`Pending Delete`**. | `MOVE` → `Pending Delete` (same locate rules) |
...
- **Retry failed:** `inbox:retryFailedRemoteOps` (bulk debug panel **Retry failed**) sets **`status = 'pending'`, `attempts = 0`, `last_error = NULL`** (and **`updated_at`**) for every row with **`status = 'failed'`**, then schedules the normal background drain. The debug panel auto-refreshes queue stats every **5s** while open and shows drain progress + ETA from the **completed** count trend over the last **30s**.
