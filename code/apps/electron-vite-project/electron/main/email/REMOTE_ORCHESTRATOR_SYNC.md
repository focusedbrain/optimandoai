...
| Pending review | `pending_review` | Add `WRDesk/PendingReview`, remove `INBOX` + `WRDesk/PendingDelete` | Move → child folder `WR Desk — Pending Review` under Inbox | `MOVE` → `WRDesk-PendingReview` |
| Pending delete | `pending_delete` | Add `WRDesk/PendingDelete`, remove `INBOX` + `WRDesk/PendingReview` | Move → child folder `WR Desk — Pending Delete` under Inbox | `MOVE` → `WRDesk-PendingDelete` |
...
- **Providers:** Treat “already moved / label missing” style errors as success where APIs are ambiguous (`skipped: true`).
