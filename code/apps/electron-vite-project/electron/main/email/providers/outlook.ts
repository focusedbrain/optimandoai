...
export class OutlookProvider extends BaseEmailProvider {
  ...
  async startOAuthFlow(): Promise<{ oauth: EmailAccountConfig['oauth']; email: string }> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanupAndResolve = (result: { oauth: EmailAccountConfig['oauth']; email: string }) => {
        if (resolved) return
        resolved = true
        if (timeoutId) clearTimeout(timeoutId)
        this.cleanup()
      ...
      this.startLocalServer()
        .then(codePromise => {
          ...
          console.log('[Outlook] Opening OAuth in system browser...')
        ...
        .then(async (tokens) => {
          ...
          cleanupAndResolve({ oauth: tokens, email })
        })
        .catch(err => {
