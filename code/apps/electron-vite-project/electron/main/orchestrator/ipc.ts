...
export function registerOrchestratorIPC(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, params: { hostUrl: string; accessToken?: string }) => {
      const hostUrl = typeof params?.hostUrl === 'string' ? params.hostUrl : ''
      const passed = typeof params?.accessToken === 'string' ? params.accessToken.trim() : ''
      const token = passed || getAccessToken() || ''
      return testRemoteOrchestratorInferenceStatus(hostUrl, token)
    },
  )
