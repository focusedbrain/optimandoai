/**
 * Rasterize PDF pages to PNG data URLs in a hidden BrowserWindow (Chromium canvas).
 * pdfjs-dist + node-canvas in the main process triggers "Image or Canvas expected" — this path does not.
 *
 * Shared by template PDF preview and Letter Viewer PDF upload.
 */
...
export type RenderedPdfPageImage = { pageNumber: number; imageDataUrl: string }

export async function renderPdfPagesToImages(absPath: string): Promise<{
  ...
  try {
    ...
    )) as { pages: RenderedPdfPageImage[]; pageCount: number }
