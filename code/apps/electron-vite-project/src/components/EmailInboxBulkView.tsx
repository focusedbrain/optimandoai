...
function parsePersistedAnalysis(json: string | null | undefined): BulkAiResultEntry | undefined {
  ...
  try {
    ...
    return {
      category: parsed.category as SortCategory,
      ...
      recommendedAction: (parsed.recommendedAction ?? 'keep_for_manual_action') as BulkRecommendedAction,
...
function BulkActionCardStructured({
  ...
  handleSendDraft: (msg: InboxMessage, draftBody: string) => void
  ...
  return (
    <div className={`bulk-action-card bulk-action-card--structured ${isExpanded ? 'bulk-action-card--expanded' : ''} ${preActionMod}`} style={{ borderLeftColor: effectiveBorderColor }}>
          ...
          >
            ...
            <div className="bulk-action-card-row-value">
              ...
              <div className="bulk-action-card-draft-actions">
                <button type="button" className="bulk-action-card-btn bulk-action-card-btn--primary" onClick={() => handleSendDraft(msg, output.draftReply ?? '')}>
      ...
      <div className="bulk-action-card-buttons">
        {(currentFilter === 'pending_delete' || currentFilter === 'pending_review' || currentFilter === 'archived') && (
          <button
            type="button"
...
export default function EmailInboxBulkView({
  ...
  const runAiCategorizeForIds = useCallback(
    ...
    [clearMultiSelect, fetchMessages, addPendingDeletePreview, addPendingReviewPreview, addArchivePreview]
  )
        ...
        return (
          <div className="bulk-action-card bulk-action-card--loading">
            ...
            <div className="bulk-action-card-actions-row">
              {(filter.filter === 'pending_delete' || filter.filter === 'pending_review' || filter.filter === 'archived') && (
                <button
                  type="button"
        ...
        <div className="bulk-view-action-group">
          {selectedCount > 0 && (
            <>
              ...
              <button
                type="button"
                className="bulk-view-review-btn"
                onClick={handleBulkMoveToPendingReview}
                ...
                {recentPendingDeleteBatches.length > 0 && (
                  <div className="bulk-view-recent-stack">
                    {recentPendingDeleteBatches.map((batch, i) => (
                      <UndoFadeWrapper key={i} onFadeComplete={() => removeRecentPendingDeleteBatch(batch.ids)}>
                        <div className="bulk-view-recent-item">
                          <span>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'stretch', gap: 12, flex: 1, minHeight: 0, overflow: 'hidden' }}>
                      ...
                      <div className="bulk-view-message-inner" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                        ...
                        <div className="bulk-view-message-bottom-bar">
                          {isUrgent && (
                            <span
