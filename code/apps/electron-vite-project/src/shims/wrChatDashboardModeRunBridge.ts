...
import { executeModeSessionRunWithInFlightGuard } from '@ext/services/modeSessionRunInFlightExecute'
...
async function executeModeSessionRunDirectForDashboard(args: {
  ...
  const run = await executeModeSessionRunWithInFlightGuard({
