...
/** UI / callers: apply when `runDraftAttachmentParseWithFallback` rejects (defense in depth). */
export function draftAttachmentParseRejectedUpdate(
  item: DraftAttachmentParseItem,
  reason: unknown,
): DraftAttachmentParseUpdate {
  const message =
    reason instanceof Error ? reason.message : typeof reason === 'string' ? reason : 'Attachment parsing failed'
  return {
    capsuleAttachment: item.capsuleAttachment,
    processing: { parsing: false, rasterizing: false, error: message },
  }
}
...
export async function runDraftAttachmentParseWithFallback(
  ...
  try {
    ...
    const hasGoodText =
      parseResult.attachment.semanticExtracted && !!parseResult.attachment.semanticContent?.trim()
    ...
    return draftAttachmentParseRejectedUpdate(item, e)
  }
}
