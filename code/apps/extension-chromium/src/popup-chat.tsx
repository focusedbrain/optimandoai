...
function PopupChatApp() {
  ...
  const handleSendBeapMessage = async () => {
    ...
    if (beapRecipientMode === 'private' && !selectedRecipient) {
      setToastMessage(beapUiValidationFailure('Please select a handshake recipient'))
      setTimeout(() => setToastMessage(null), 3000)
      return
    }
    ...
    if (beapDraftAttachments.some((a) => a.processing?.parsing)) {
      setToastMessage(beapUiValidationFailure('Wait for attachment extraction to finish before sending.'))
  ...
  const renderEmailComposeContent = () => {
    ...
    const handleEmailComposeFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (tooBig.length) {
        setToastMessage(
          beapUiValidationFailure(formatOversizeAttachmentRejection(tooBig.map((f) => f.name))),
        )
        setTimeout(() => setToastMessage(null), 10000)
      }
