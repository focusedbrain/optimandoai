 ...
 * Injected via chrome.scripting.executeScript — must be one self-contained function
 * (no module-level closures) so Chrome can serialize it into the tab.
 */

import type { DomSlotCapture, DomSnapshot } from '../types/optimizationTypes'

/**
 * Serializable entry point for `chrome.scripting.executeScript({ func, args })`.
 * All helpers and constants are nested so the function has no module-level closures.
 */
export function domSnapshotCaptureInjected(gridRootSelector = '#grid-root'): DomSnapshot | null {
  ...
  const sel = typeof gridRootSelector === 'string' && gridRootSelector.trim() ? gridRootSelector.trim() : '#grid-root'
...
/** Same logic as {@link domSnapshotCaptureInjected} for non-injected callers. */
export function captureDomSnapshot(gridRootSelector = '#grid-root'): DomSnapshot | null {
  return domSnapshotCaptureInjected(gridRootSelector)
}
