/**
 * HsContextDocumentReader — Premium page-by-page document reader (extension)
 *
 * Uses hsContextProfilesRpc (WebSocket → Electron) for document page APIs.
 */
...
export const HsContextDocumentReader: React.FC<HsContextDocumentReaderProps> = ({
  ...
  const [searchMatches, setSearchMatches] = useState<Array<{ page_number: number; match_count: number; snippet: string }>>([])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getDocumentPageList(documentId),
    ])
      .then(([count, pages]) => {
        if (cancelled) return
        setPageCount(count)
  ...
  useEffect(() => {
    ...
    setPageText(null)
    let cancelled = false
    getDocumentPage(documentId, currentPage).then((text) => {
      if (cancelled) return
      const t = text ?? ''
      setPageText(t)
      setPageCache((prev) => ({ ...prev, [currentPage]: t }))
    })
    return () => { cancelled = true }
  }, [documentId, currentPage, pageCount, pageCache])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      searchDocumentPages(documentId, searchQuery.trim()).then((matches) => {
  ...
  const handleDownloadFullText = useCallback(async () => {
    try {
      const text = await getDocumentFullText(documentId)
      if (!text) return
