...
export function exportAgentToJson(agentConfig: any): AgentExportFormat {
  ...
  if (agentConfig.reasoning) {
    ...
    exported.reasoning = {
      ...
      outputFormattingInstructions: wrapWithSchema(ReasoningElementsSchema.outputFormattingInstructions, reasoning.outputFormattingInstructions || ''),
  ...
  if (agentConfig.reasoningSections && agentConfig.reasoningSections.length > 0) {
    exported.reasoningSections = agentConfig.reasoningSections.map((section: any, index: number) => ({
      ...
      outputFormattingInstructions: wrapWithSchema(ReasoningElementsSchema.outputFormattingInstructions, section.outputFormattingInstructions || ''),
  ...
  try {
    ...
    if (data.reasoning) {
      agent.reasoning = {
        ...
        outputFormattingInstructions: extractValue(data.reasoning.outputFormattingInstructions) || '',
    ...
    if (data.reasoningSections && Array.isArray(data.reasoningSections)) {
      agent.reasoningSections = data.reasoningSections.map((section: any) => ({
        ...
        outputFormattingInstructions: extractValue(section.outputFormattingInstructions) || '',
