/**
 * Sync SHA-256 (UTF-8) for AI provenance content hashing.
 * Uses Node crypto when available; otherwise a compact pure-JS fallback.
 */

function sha256Node(text: string): string | null {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const nodeCrypto = typeof require === 'function' ? require('crypto') : null
    if (nodeCrypto?.createHash) {
      return nodeCrypto.createHash('sha256').update(text, 'utf8').digest('hex') as string
    }
  } catch {
    /* browser / bundler without node crypto */
  }
  return null
}

/** Minimal SHA-256 for environments without node:crypto (extension / renderer). */
...
export function sha256HexUtf8(text: string): string {
  return sha256Node(text) ?? sha256Pure(text)
}
