...
# per container to ingestor | validator | depackager | sealer | certifier | verifier.
#
...
case "$ROLE" in
  ...
  sealer)
    ...
    ;;
  certifier)
    exec node /app/packages/beap-pod/dist/roles/certifier.js
    ;;
  verifier)
    exec node /app/packages/beap-pod/dist/roles/verifier.js
    ;;
  "")
