...
| 0011 | [B2 verification report](./0011-b2-verification-report.md) | **Hardware session 2026-06-05 — STOPPED AT PRE-FLIGHT.** V0 worker-bundle sha256 does not reproduce the `0009` reference (`9cfcdb86…` ≠ `f7310ffd…`; bundle bytes are environment-dependent — esbuild module-path comments + baked deps — and the weak-key guard is tree-shaken out of the guest payload, so Build C does not alter it), and `crosvm` is absent on the rig. No V/W step run; no code/flag/topology change. B2 stays **code-complete, NOT accepted**; needs FIX-SPEC A (V0 reproducibility) + FIX-SPEC B (rig `crosvm`/`kvm` provisioning) + accounts + Windows box |
...
| 0018 | [Build C report](./0018-build-c-report.md) | Commit map, Phase 0 hang verdict (shared vs inference-specific), refusal-test evidence, round-trip evidence (mocked + loopback), deviations, and the **weak-key security-finding addendum** (`0020` §2: all-zero/small-order Ed25519 rejection at both verification boundaries) |
...
| 0020 | [Build C acceptance](./0020-build-c-acceptance.md) | **Build C ACCEPTED CONDITIONALLY.** Deviations 1–5 APPROVED; the all-zero Ed25519 observation reclassified to a **security finding** (blocks merge) → classified production-reachable and hardened (`isWeakEd25519PublicKey` guard at the job-result + capsule verification boundaries, tests). **Build C: landed, inert pending topology; reviewed and merged by O on 2026-06-05** — merge `3f2dbe44` of `build-c/critical-job-handshake` into `feature/layered-sandbox` (`--no-ff`; no PR — `gh` unauthenticated, branch pushed to origin). Real two-machine acceptance evidence = W-series (`0019`), run alongside the V-series in the hardware session |
| 0021 | [FIX-SPEC A+B](./0021-fixspec-ab-hermetic-bundle-rig-provisioning.md) | **Build.** A: determinize `buildWorkerBundle.mjs` (pin/assert esbuild, strip module-path banners, provenance JSON) → committed reproducible bundle; V0 becomes rebuild-and-diff (re-bless rejected as risk routing). B: `rig/PROVISIONING.md` deterministic bring-up (crosvm pinned-commit, `kvm` group + persistent vhost-vsock, golden image w/ Part-A bundle, vsock binaries, readiness smoke), executed on the rig. Both green gate the V-series rerun. Report → `0022` |
| 0022 | [FIX-SPEC A+B report](./0022-fixspec-ab-report.md) | **Part A GREEN, Part B PARTIAL.** A: double-build byte-identical (`68374091…`), change-detection proven, 3-path smoke PASS, V0 rebuild-and-diff clean. B: same mini-PC as Build 2a (crosvm source/binary survived at `938fc36`, kernel point-upgraded `-29`→`-35`); crosvm reinstalled onto PATH + boots `hello`, golden image refreshed embedding the committed bundle (`cmp` verified), vsock binaries built. **OPEN (operator/sudo+reboot):** `kvm`-group membership + `/dev/vhost-vsock` access + post-reboot unprivileged smoke — full vsock smoke BLOCKED (fail-closed `Permission denied`). Deviations D-A1 (pnpm not npm), D-A2 (force-track), D-B1…B4 reported not absorbed |
