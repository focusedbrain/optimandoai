...
| 0020 | [Build C acceptance](./0020-build-c-acceptance.md) | **Build C ACCEPTED CONDITIONALLY.** Deviations 1–5 APPROVED; the all-zero Ed25519 observation reclassified to a **security finding** (blocks merge) → classified production-reachable and hardened (`isWeakEd25519PublicKey` guard at the job-result + capsule verification boundaries, tests). PR `build-c/critical-job-handshake` → `feature/layered-sandbox` for O to review + merge. **Post-merge status:** Build C landed, **inert pending topology configuration**; real two-machine acceptance evidence = W-series (`0019`), run alongside the V-series in the hardware session |
