 ...
 * Version 0.1.2 * platform-neutral: runs on any PHP hosting (PHP 7.2+), independent
...
function wrc_setup(string $host): array {
    ...
    wrc_save_state($s);
    // Also store the admin key where it is reachable via FTP (in case the bookmark is lost):
    @file_put_contents(WRC_DATA_DIR . '/admin-key.txt', $s['admin_key'] . "\n");
...
function wrc_page(string $title, string $body): void {
    ...
    echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
       ...
       . '</style></head><body>' . $body . '<p style="margin-top:48px;color:#8a93a3;font-size:12px">WR Connect 0.1.2 &middot; Optirando&trade; Public Handshake &middot; keys never leave this server</p></body></html>';
    exit;
}
...
if ($s === null) {
    ...
    wrc_page('WR Connect &mdash; set up', '<h1>&check; Setup complete</h1>'
        ...
        . '<code>wr-connect-data/admin-key.txt</code> on your web space (readable via FTP). '
...
if ($sess === null) {
    wrc_page('WR Connect &mdash; sign in', '<h1>WR Connect</h1>'
        ...
        . '(shown once at setup; also stored in <code>wr-connect-data/admin-key.txt</code>, readable via FTP / file manager).</p>'
        . wrc_login_form($self));
}
...
wrc_page('WR Connect &mdash; ' . $host,
  ...
  . '<div class="note"><small>Key IDs: ' . e($s['root']['kid'] . ' &middot; ' . $s['sbk']['kid']) . '. The private keys live exclusively in <code>wr-connect-data/</code> on this server and cannot be retrieved via browser. A later update adds the login/logout binding &mdash; nothing further to prepare.</small></div>'
);
